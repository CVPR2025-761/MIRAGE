...
def init_downstream_model(config, index=0):
    if config.task == 'zero_shot_classification':
        log.info(f"Instantiating classification_model <{config.zero_shot_classification_model._target_}>")
        model = hydra.utils.instantiate(config.zero_shot_classification_model)
    elif config.task == 'vqa':
        log.info(f"Instantiating vqa_model <{config.vqa_model._target_}>")
        model = hydra.utils.instantiate(config.vqa_model)
    elif config.task == 'mc_vqa':
        log.info(f"Instantiating mc_vqa_model <{config.mc_vqa_model._target_}>")
        model = hydra.utils.instantiate(config.mc_vqa_model)
    elif config.task == 'retrieval':
        log.info(f"Instantiating classification_model <{config.retrieval_model._target_}>")
        model = hydra.utils.instantiate(config.retrieval_model)
    elif config.task == 'knn':
        log.info(f"Instantiating classification_model <{config.knn_model._target_}>")
        model = hydra.utils.instantiate(config.knn_model)
    elif config.task == 'text_knn':
        log.info(f"Instantiating classification_model <{config.text_knn_model._target_}>")
        model = hydra.utils.instantiate(config.text_knn_model)
    return model
